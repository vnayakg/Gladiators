...
function App() {
  ...
  const loadNotes = () => {
    ...
    noteRef
      ...
      .then((s) => {
        let savedNotes = [];
        s.forEach((data) => {
          savedNotes = [...savedNotes, data.data()];
        });
  ...
  useEffect(() => {
    if (submissionCheckerId && submissionStatus === compeletedStatus) {
      ...
      axios
        .get(`https://api.paiza.io/runners/get_details?${querystring}`)
        .then((res) => {
  ...
  const updateSubmissionStatus = () => {
    ...
    const querystring = params.toString();
    axios
      .get(`https://api.paiza.io/runners/get_status?${querystring}`)
      .then((res) => {
  ...
  const Resources = () => {
    return (
      <div>
        ...
        {["Java", "Python", "C", "C++", "Javascript"].map((lang, ind) => {
          return (
            <button
              ...
              value={lang}
              onClick={(lang) => setResLang(lang.target.innerText)}
              key={ind}
            >
              <b>{lang}</b>
            </button>
        ...
        <CardList
          resources={ResourcesList.filter(
            (itm) =>
              resLang.toLowerCase().includes(itm.name.toLowerCase()) ||
              resLang.toLowerCase().includes(itm.topic[0].toLowerCase())
          )}
        />
      </div>
    );
        ...
        {isLoggedin ? (
          <>
              ...
              <div>
                ...
                <div className="col-lg-6 col-sm-12 mb-2 mt-3">
                  <div className="d-flex align-items-center justify-content-between mb-2">
                    <label
                      for="file-upload"
                      class="custom-file-upload btn-outline-success"
                    >
                  ...
                  <div>
                    <p>
                      <i>Note: after selecting image press get code</i>
                    </p>
                  </div>
            ...
            <Route path="/resources" component={Resources}>
              <div>
                <Search handleSearchNote={setResLang} />
                {["Java", "Python", "C", "C++", "Javascript"].map(
                  (lang, ind) => {
                    return (
                      <button
                        className="btn btn-outline-primary mx-3 my-3"
                ...
                <CardList
                  resources={ResourcesList.filter(
                    (itm) =>
                      itm.topic[0].toLowerCase() === resLang.toLowerCase()
                  )}
                />
              </div>
            </Route>
